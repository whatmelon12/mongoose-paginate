...
const MONGO_URI = "mongodb://127.0.0.1/mongoose_paginate_test?authSource=admin";
  ...
  describe("paginates", function() {
    ...
    it("with populate", function() {
      return Book.paginate(
        {},
        { autopopulate: false, populate: "author" }
      ).then(function(result) {
        expect(result.docs[0].author.name).to.equal("Arthur Conan Doyle");
      });
    });

    it("with autopopulate {true}", function() {
      return Book.paginate({}, { autopopulate: true }).then(function(result) {
        expect(result.docs[0].author.name).to.equal("Arthur Conan Doyle");
      });
    });

    it("with autopopulate {false}", function() {
      return Book.paginate({}, { autopopulate: false }).then(function(result) {
        expect(result.docs[0].author.toString()).to.equal(authorId);
      });
    });
